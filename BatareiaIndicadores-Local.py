...
def concentracion(df,column,periodo):    
    ...
    df.insert(3,'participacion',Participacion(df,column))
    num=[]
    num.append(len(df['participacion'].values.tolist()))
    cols=[f'CR{i}' for i in range(1, max(num)+1)]
    cols.insert(0,'PERIODO')
    Conc=pd.DataFrame(columns=cols)
    part=sorted(df[df['periodo']==periodo]['participacion'].values.tolist(),reverse=True)
    cum=Cumulative(part)
    cum.insert(0, periodo)
    if len(cum) <max(num)+1:
        compl=[1.0]*(max(num)+1-len(cum))
        cum.extend(compl)
    else:
        pass
    Conc_length = len(Conc)
    Conc.loc[Conc_length] = cum
    return Conc
##
...
if select_mercado == 'Telefonía local':   
    ...
    if select_dimension == 'Nacional':
        ...
        if select_indicador == 'Concentración':
            ...
            ConcIng=pd.concat(dflistIng).fillna(1.0).reset_index().drop('index',1) 
            ConcLin=pd.concat(dflistLin).fillna(1.0).reset_index().drop('index',1) 
                     
            conc=st.slider('Seleccionar nivel concentración ', 1,17,1,1)
            ...
            if select_variable == "Tráfico":
                ...
                st.write(ConcTraf.style.apply(f, axis=0, subset=[colsconTraf[conc]]))
                st.plotly_chart(fig4,use_container_width=True)
            if select_variable == "Ingresos":
                colsconIng=ConcIng.columns.values.tolist()
                st.write(ConcIng.style.apply(f, axis=0, subset=[colsconIng[conc]]))
                st.plotly_chart(fig5,use_container_width=True)
            if select_variable == "Líneas":
                colsconLin=ConcLin.columns.values.tolist()
                st.write(ConcLin.style.apply(f, axis=0, subset=[colsconLin[conc]]))
        ...
        if select_indicador == 'IHH':
            ...
            for elem in PERIODOS:
                ...
                prTr.insert(4,'IHH',round(sum([elem**2 for elem in prTr['PARTICIPACION'].values.tolist()]),2))
                ...
                prIn.insert(4,'IHH',round(sum([elem**2 for elem in prIn['PARTICIPACION'].values.tolist()]),2))
                ...
                prLi.insert(4,'IHH',round(sum([elem**2 for elem in prLi['PARTICIPACION'].values.tolist()]),2))
